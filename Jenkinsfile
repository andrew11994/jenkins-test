        ...
        stage('execute shell') {
            ...
            steps {
                script {
                  ...
                  def raw_repos= [sh(script: "curl -u ${GH_TOKEN}:x-oauth-basic -s ${env.GITHUB_BASE_URL}/orgs/${env.SRC_GH_ORG}/repos | jq '.[] | .name' | grep -v ${env.TARGET_BLACKLIST}", returnStdout: true)]
